...
A `Dockerfile` and `fly.toml` are checked in. From a terminal with
[`flyctl`](https://fly.io/docs/hands-on/install-flyctl/) installed:

```bash
fly auth signup     # or: fly auth login
fly launch --copy-config --name knightfall   # uses the existing fly.toml
fly secrets set \
  MONGODB_URI="mongodb+srv://..." \
  NEXTAUTH_URL="https://knightfall.fly.dev" \
  NEXTAUTH_SECRET="$(openssl rand -base64 32)" \
  NEXT_PUBLIC_SOCKET_URL="https://knightfall.fly.dev"
fly deploy
```

The default `fly.toml` requests one shared 256 MB VM in `iad` and lets Fly
auto-stop it when idle (set `min_machines_running = 1` to keep it always-on
within the free allowance).

### Other Node hosts

Any host that runs `npm start` will work — Railway, a small VPS, etc.:
